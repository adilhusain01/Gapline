...
import { addresses, useMarkets, useSessionStatus, useSettlementRound, useShareBalances } from "@/lib/gapline";
...
function MarketPage() {
  ...
  const selected =
    markets.find((market) => market.id === selectedMarketId) ?? open[0] ?? markets[0];
  ...
  const canResolve =
    selected && !selected.resolved && Number(selected.reopenTs) * 1000 <= Date.now() && settlementRound !== undefined;

  return (
    <div className="space-y-6">
      <PricePanel active={status.activeMarketId !== undefined ? markets.find((m) => m.id === status.activeMarketId) : selected} />
      ...
      {isLoading ? (
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
          <Card>
            <CardHeader className="flex-row items-start justify-between space-y-0">
              <div className="space-y-1">
                ...
                <p className="text-sm text-muted-foreground">
                  Friday close {usd(selected.refPrice)} - settles on the first feed price after the session
                  reopens
                </p>
              </div>
            ...
            <CardContent className="space-y-4">
              <MarketRanges market={selected} selected={outcome} onSelect={setOutcome} shares={shares} />
              <div className="flex flex-wrap items-center gap-x-6 gap-y-1 text-xs text-muted-foreground">
                <span>depth b = {Number(selected.liquidity) / 1e18}</span>
                <span>collateral {usdg(selected.collateralHeld)}</span>
                {selected.resolved ? <span>settled at {usd(selected.settlePrice)}</span> : null}
              </div>
              {canResolve ? (
                <Button
                  variant="secondary"
          ...
          <div className="space-y-6">
            ...
            {status.isFeedFrozen && open.length === 0 ? <OpenMarketCard /> : null}
          </div>
        </div>
      ) : status.isFeedFrozen ? (
        <OpenMarketCard />
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base font-medium">No weekend market yet</CardTitle>
          </CardHeader>
          <CardContent className="text-sm text-muted-foreground">
            TSLA is trading and the Chainlink feed is live, so there is nothing to price. A market can be
            opened as soon as the session closes on Friday at 20:00 ET.
          </CardContent>
        </Card>
      )}
            ...
            {markets.map((market) => (
              ...
              >
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  ...
                  {new Date(Number(market.reopenTs) * 1000).toUTCString().slice(0, 16)}
                </p>
              </button>
            ))}
          </CardContent>
