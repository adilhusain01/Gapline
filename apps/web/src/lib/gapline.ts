...
const calendar = { address: addresses.calendar, abi: marketCalendarAbi, chainId } as const;
const oracle = { address: addresses.oracle, abi: impliedPriceOracleAbi, chainId } as const;
const feed = { address: addresses.feed, abi: mirroredFeedAbi, chainId } as const;
const market = { address: addresses.gapMarket, abi: gapMarketAbi, chainId } as const;
const pool = { address: addresses.pool, abi: gapGuardedLendingPoolAbi, chainId } as const;

export type PriceBand = { low: bigint; mid: bigint; high: bigint; implied: boolean };
...
export function useSessionStatus() {
  ...
  const [isOpen, nextOpen, lastClose, frozen, band, round, activeMarketId, hasActiveMarket] =
    query.data ?? [];
  ...
  return {
    ...
    band: band?.result
      ...
      : undefined,
    feedPrice: round?.result ? (round.result as readonly bigint[])[1] : undefined,
    feedUpdatedAt: round?.result ? (round.result as readonly bigint[])[3] : undefined,
    activeMarketId: hasActiveMarket?.result ? (activeMarketId?.result as bigint) : undefined,
  };
}
...
export function useMarkets() {
  ...
  const markets: MarketView[] = useMemo(() => {
    ...
    return ids
      .map((id, index) => {
        const raw = details.data[index * 3]?.result as Record<string, unknown> | undefined;
        const prices = details.data[index * 3 + 1]?.result as readonly bigint[] | undefined;
        const move = details.data[index * 3 + 2]?.result as readonly bigint[] | undefined;
  ...
  return { markets, isLoading: count.isLoading || details.isLoading, refetch: details.refetch };
}
...
export function useWalletState() {
  ...
  const query = useReadContracts({
    contracts: [
      { address: addresses.usdg, abi: erc20Abi, chainId, functionName: "balanceOf", args: [address ?? "0x0"] },
      { address: addresses.usdg, abi: erc20Abi, chainId, functionName: "allowance", args: [address ?? "0x0", addresses.gapMarket] },
      { address: addresses.usdg, abi: erc20Abi, chainId, functionName: "allowance", args: [address ?? "0x0", addresses.pool] },
      { address: addresses.stock, abi: erc20Abi, chainId, functionName: "balanceOf", args: [address ?? "0x0"] },
      { address: addresses.stock, abi: erc20Abi, chainId, functionName: "allowance", args: [address ?? "0x0", addresses.pool] },
    ],
    query: { enabled: Boolean(address) },
  });
  const [usdgBalance, marketAllowance, poolAllowance, stockBalance, stockAllowance] = query.data ?? [];
  return {
    ...query,
...
export function usePosition() {
  ...
  const [collateral, debt, maxBorrow, liquidatable, riskPrice, liquidationPrice] = query.data ?? [];
  return {
    ...query,
...
export function useShareBalances(marketId: bigint | undefined, outcomes: number) {
  const { address } = useAccount();
  const query = useReadContracts({
    ...
    query: { enabled: Boolean(address) && marketId !== undefined && outcomes > 0 },
  });
  return (query.data?.map((entry) => (entry.result as bigint | undefined) ?? 0n) ?? []) as bigint[];
}
...
export function useSettlementRound(market: MarketView | undefined) {
  ...
  const rounds = useReadContracts({
    contracts: candidates.map((round) => ({ ...feed, functionName: "getRoundData" as const, args: [round] })),
...
export { gapMarketAbi, gapGuardedLendingPoolAbi, impliedPriceOracleAbi, chainId };
