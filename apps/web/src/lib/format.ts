...
export function usd(value: bigint | undefined, decimals = PRICE_DECIMALS, digits = 2) {
...
export function rangeLabel(edges: readonly bigint[], index: number) {
  ...
  if (index === edges.length) return `${percent(bpsToPercent(edges[edges.length - 1]), 2)} or better`;
