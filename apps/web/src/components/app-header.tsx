...
function ConnectButton() {
  ...
  if (!isConnected) {
    ...
    return (
      <Button size="sm" disabled={isPending || !connector} onClick={() => connector && connect({ connector })}>
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end">
        <DropdownMenuItem
          onClick={() =>
            window.open(`${robinhoodTestnet.blockExplorers.default.url}/address/${address}`, "_blank")
          }
        >
          View on explorer
        </DropdownMenuItem>
    ...
    <header className="sticky top-0 z-30 border-b bg-background/80 backdrop-blur">
      <div className="mx-auto flex h-14 w-full max-w-6xl items-center gap-4 px-4">
        <Link to="/" className="flex items-baseline gap-2">
          ...
          <span className="hidden text-xs text-muted-foreground sm:inline">weekend price for tokenized stocks</span>
        </Link>
