...
import { addresses, chainId, useSessionStatus, useWalletState, type MarketView } from "@/lib/gapline";
...
export function TradePanel({
  ...
  const tradingClosed = market.resolved || Number(market.reopenTs) * 1000 <= Date.now();
    ...
    <Card>
      <CardHeader className="space-y-1">
        <CardTitle className="text-base font-medium">
          {market.resolved ? "Market settled" : tradingClosed ? "Trading closed" : "Trade this range"}
        </CardTitle>
        <p className="text-sm text-muted-foreground">{rangeLabel(market.edges, outcome)}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {market.resolved ? (
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">
              Every winning share redeems for 1 USDG. Losing shares expire worthless.
            </p>
            <Button
              className="w-full"
          ...
          <>
            ...
            <div className="flex items-baseline justify-between text-sm">
              <span className="text-muted-foreground">{side === "buy" ? "Cost" : "You receive"}</span>
              <span className="font-mono tabular-nums">{quote.isLoading ? "..." : usdg(cost)}</span>
            </div>
            <div className="flex items-baseline justify-between text-sm">
            ...
            <Button
              className="w-full"
              disabled={!isConnected || tradingClosed || size === 0n || Boolean(pending)}
              onClick={submit}
            >
              {pending ? <Loader2 className="size-4 animate-spin" /> : null}
              {!isConnected
