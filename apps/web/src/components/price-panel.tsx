					...
					<TooltipTrigger asChild>
						<div>
							<Stat
								...
								hint={
									implied
										? "borrowing uses the low end, liquidation the high end"
										: "no band while the feed is live"
								}
							/>
						</div>
					</TooltipTrigger>
					<TooltipContent className="max-w-64">
						Two standard deviations of the market's implied move. Lenders read
						this instead of a frozen feed.
					</TooltipContent>
				</Tooltip>
			...
			{!status.isOpen && !implied ? (
				<CardContent className="pt-0">
					<p className="flex items-center gap-2 rounded-md border border-destructive/30 bg-destructive/5 p-3 text-sm text-destructive">
						...
						The feed is frozen and no weekend market is pricing it. Lending is
						paused until one opens.
					</p>
				</CardContent>
			) : null}
		</Card>
