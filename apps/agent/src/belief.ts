...
/**
 * Probability of each reopening range under a normal model of the gap. The mean blends the stock's Uniswap price
 * (when a deep enough pool is available) with the weekend BTC move scaled by the stock's fitted beta, plus the
 * analyst's bounded drift. The 50/50 blend had the lowest error in the backtests (docs/backtest-<symbol>.md).
 * `config` is the stock's config (stockConfig(symbol)).
 */
